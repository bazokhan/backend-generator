...
jest.mock('@tg-scripts/io/utils/user-prompt', () => ({
  ...
  promptText: jest
    .fn()
    .mockImplementation((_q: string, opts?: { defaultValue?: string }) => Promise.resolve(opts?.defaultValue ?? '')),
}));
...
describe('CommandLineInterface', () => {
  ...
  it('prints init subcommand help when tgraph init --help is provided', async () => {
    const cli = new CommandLineInterface({ configLoader: createLoaderMock(), cliParser: new CliParser() });
  ...
  it('does not overwrite configuration when init is executed twice', async () => {
    ...
    const writeFileMock = jest.fn();
    const existsSyncMock = jest.fn().mockReturnValue(true); // File exists at target path
  ...
  it('runs swagger command before generating types', async () => {
    ...
    expect(execSyncMock).toHaveBeenCalledWith(
      expect.stringContaining('swagger-typescript-api'),
      expect.objectContaining({ cwd: expect.any(String), stdio: 'inherit' }),
    );
  });
  ...
  it('skips swagger command when --skip-swagger is provided', async () => {
    ...
    // Now includes formatting commands for api.ts and swagger.json
    expect(commands.length).toBeGreaterThanOrEqual(1);
    expect(commands[0]).toContain('swagger-typescript-api');
    // Verify formatting commands are also called
  ...
  it('forces public mode when --public is provided', async () => {
    ...
    expect(runtimeConfig.api.authenticationEnabled).toBe(false);
    expect(runtimeConfig.api.requireAdmin).toBe(false);
  });

  it('respects non-interactive mode from config', async () => {
    const loader = createLoaderMock({
...
describe('CommandLineInterface - doctor command', () => {
  ...
  it('runs doctor command with warnings but no errors', async () => {
    ...
    systemValidatorRunDiagnosticsMock.mockResolvedValue({
      categories: [
        {
          name: 'Configuration',
          results: [{ severity: 'ok', message: 'Config file found' }],
        },
        {
          name: 'Project Paths',
          results: [
  ...
  it('runs doctor command with errors', async () => {
    ...
    systemValidatorRunDiagnosticsMock.mockResolvedValue({
      categories: [
        {
          name: 'Configuration',
          results: [{ severity: 'error', message: 'No configuration file found', suggestion: "Run 'tgraph init'" }],
        },
        {
          name: 'Prisma Schema',
          results: [{ severity: 'error', message: 'Schema file not found', suggestion: "Run 'npx prisma init'" }],
        },
      ],
      hasErrors: true,
      hasWarnings: false,
  ...
  it('runs doctor without config file and reports error', async () => {
    ...
    expect(callArg?.input.prisma.schemaPath).toBe('');
  });
  ...
  it('handles errors during diagnostic execution', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining('Error running diagnostics'),
      expect.any(Error),
    );
  });
