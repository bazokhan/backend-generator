...
import { SystemValidator, DiagnosticReport } from '../io/validation/SystemValidator';
...
describe('SystemValidator', () => {
  ...
  const createMockConfig = (overrides: MockConfigOverrides = {}): Config => {
    ...
    if (overrides.input) {
      base.input = {
        ...base.input,
        ...overrides.input,
        prisma: overrides.input.prisma ? { ...base.input.prisma, ...overrides.input.prisma } : base.input.prisma,
        dashboard: overrides.input.dashboard
          ? { ...base.input.dashboard, ...overrides.input.dashboard }
    ...
    if (overrides.output) {
      base.output = {
        ...base.output,
        ...overrides.output,
        backend: overrides.output.backend
          ? { ...base.output.backend, ...overrides.output.backend }
          : base.output.backend,
    ...
    if (overrides.api) {
      base.api = { ...base.api, ...overrides.api };
    }
    ...
    if (overrides.schemaPath !== undefined) {
      base.input.prisma.schemaPath = overrides.schemaPath;
    }

    if (overrides.dtosPath !== undefined) {
      base.output.backend.dtosPath = overrides.dtosPath;
    }
    ...
    if (overrides.dashboardResourcesPath !== undefined) {
      base.output.dashboard.resourcesPath = overrides.dashboardResourcesPath;
    }
  ...
  describe('checkConfiguration', () => {
    it('should validate all config fields correctly', async () => {
      ...
      expect(results.some((r) => r.message.includes('Prisma schema file'))).toBe(true);
      expect(results.some((r) => r.message.includes('generated DTOs'))).toBe(true);
      expect(results.some((r) => r.message.includes('generated modules'))).toBe(true);
      expect(results.some((r) => r.message.includes('root directory where the generated backend'))).toBe(true);
      expect(results.some((r) => r.message.includes('generated dashboard will be stored'))).toBe(true);
      expect(results.some((r) => r.message.includes('generated dashboard resources'))).toBe(true);
      expect(results.some((r) => r.message.includes('Suffix for the generated API classes'))).toBe(true);
      expect(results.some((r) => r.message.includes('API route prefix'))).toBe(true);
      expect(results.some((r) => r.message.includes('Authentication'))).toBe(true);
      expect(results.some((r) => r.message.includes('React Admin components'))).toBe(true);
      expect(results.some((r) => r.message.includes('non-interactive mode'))).toBe(true);
    });
    ...
    it('should handle empty suffix correctly', async () => {
      ...
      const suffixResult = results.find((r) => r.message.includes('Suffix for the generated API classes'));
