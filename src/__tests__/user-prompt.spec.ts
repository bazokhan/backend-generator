...
import { promptUser } from '../io/utils/user-prompt';
...
describe('promptUser', () => {
  ...
  describe('no responses', () => {
    ...
    it('should re-prompt for empty string without default, then accept n', async () => {
      const promise = promptUser('Test question?');
      // First call with empty string triggers re-prompt
      questionCallback('');
      // Second call with 'n' should resolve
      questionCallback('n');
      const result = await promise;

      expect(result).toBe(false);
      expect(mockRl.question).toHaveBeenCalledTimes(2); // Called twice due to re-prompt
    });
