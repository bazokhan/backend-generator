...
describe('AdapterParser', () => {
  ...
  describe('discoverAdapters', () => {
    ...
    it('should discover .adapter.js files', () => {
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      (fs.readdirSync as jest.Mock).mockReturnValue(['test.adapter.js', 'another.adapter.ts']);
...
`;
    ...
    it('should parse adapter file successfully', async () => {
      ...
      const result = await parser.parseAdapter('/test/path/create-with-slug.adapter.ts', '/test');
...
`;
  ...
  describe('parseAdapters', () => {
    it('should parse all adapters for a model', async () => {
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      (fs.readdirSync as jest.Mock).mockReturnValue(['adapter1.adapter.ts', 'adapter2.adapter.ts']);
...
`);
    ...
    it('should skip invalid adapters', async () => {
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      (fs.readdirSync as jest.Mock).mockReturnValue(['valid.adapter.ts', 'invalid.adapter.ts']);
