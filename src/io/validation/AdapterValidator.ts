...
export class AdapterValidator {
  private static readonly VALID_HTTP_METHODS: HttpMethod[] = [
    'GET',
    'POST',
    'PUT',
    'DELETE',
  ...
  private validateConfig(adapter: AdapterDefinition): void {
    ...
    if (config.select && config.include) {
      throw new AdapterValidationError(
        name,
        'Cannot specify both "select" and "include". Choose one.'
      );
    }
  }
    ...
    if (typeof config.auth === 'string') {
      ...
      for (const guard of config.auth) {
        if (typeof guard !== 'string' || !guard.trim()) {
          throw new AdapterValidationError(
            name,
            'All auth guards must be non-empty strings'
          );
        }
      }
      ...
      throw new AdapterValidationError(
        name,
        'Auth must be a string, array of strings, or undefined'
      );
    }
  }
  ...
  private validateHandlerCode(adapter: AdapterDefinition): void {
    ...
    if (!handlerCode.includes('(ctx)') && !handlerCode.includes('(context)')) {
      throw new AdapterValidationError(
        name,
        'Handler must accept a context parameter (ctx or context)'
      );
    }
    ...
    if (!hasReturn) {
      throw new AdapterValidationError(
        name,
        'Handler must return a value (either { args } or adapter.response())'
      );
    }
  }
   ...
   */
  private validateNoDuplicatePaths(
    adapters: AdapterDefinition[],
